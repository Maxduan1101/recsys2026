...
0.5. **Public blind diversity bottleneck**
   ...
   - Current primary after weighted four-model RRF and response-mix tuning: `experiments/goalflow_ens_ltr120_140_200_col1_lambda2_w140half_w20013_rrf26_judge_clean_mix_clean/blindset_A/submission.zip`.
...
6. **LightGBM/CatBoost LambdaRank design**
   ...
   - Implemented weighted model-level RRF in `scripts/ensemble_predictions.py`. The best tested setting is now `rrf_k=26` with weights `[1.0, 0.5, 1.3, 1.0]`, improving official OOF to `nDCG@20=0.183924`; Blind-A-shaped validation versus equal-weight is positive on the 2000-panel check, so it becomes the current first-choice package while the earlier weighted and equal-weight ensembles remain fallbacks.
   - Pro round7 answers saved under `research/pro_answers/round7/`: keep `judge_clean_mix` as the next real response style, use anchored global weighted RRF before any stacker/gating model, keep train-context and embedding signals gated or tail-only until they pass strict blind-like uncertainty checks, and treat close ranking wins as backups if confidence intervals overlap zero.
   - Pro round8 answers saved under `research/pro_answers/round8/`: submit the weighted RRF ranking, stop trying to buy catalog diversity with ranking perturbations on Blind A, and treat embeddings as gated consistency evidence rather than a new head-ranking source.
   ...
   - Rejected position-band weighted RRF: head/tail weighting variants did not improve on the global weighted RRF score `0.183923885`.
   - Rejected anchored weighted RRF for promotion: a tiny equal-weight anchor reached `0.183956`, only `+0.000032` above the current weighted package and below the required margin for another submission candidate.
   - Implemented optional `--embedding-features` in `scripts/run_ltr_rerank.py` for seed/user official-embedding LTR features without rebuilding candidate caches. First-fold probes reject promotion: `track_cf + user_cf` `0.18341`, metadata seed `0.18301`, attributes seed `0.17892`. Keep the code as a diagnostic path for future gated features.
   - Rejected a hard quoted-title promotion rule: promoting top-20 candidates whose title matched quoted user text changed hundreds of rows, improved only 5, hurt 120+ rows, and dropped OOF nDCG by `0.0021` to `0.0032`.
   - Rejected an ultra-conservative batch-level repeat repair on the current weighted RRF package. Keeping ranks 1-19 and replacing only rank 20 raised dev catalog diversity by `+0.01476`, but still dropped OOF nDCG@20 from `0.183924` to `0.183497`; stronger repairs lost up to `0.00094` nDCG. This confirms the remaining catalog headroom is not worth ranking perturbation before a public signal says otherwise.
   - Rejected a second local RRF weight probe: `rrf_k=28`, weights `[1.0, 0.45, 1.45, 1.05]` raised official OOF only to `0.183966` (`+0.000043`) and failed Blind-A-shaped validation with 1000-panel mean delta `-0.000096` and p10 `-0.00109`.
   - Implemented `scripts/apply_consensus_fallback.py`. The current top-1 support fallback raises official OOF to `0.183986` and Blind-A-shaped 5000-panel mean delta to `+0.000278`, but it changes `0 / 80` Blind A rows, so it is a future Blind-B/future-split option rather than a current submission replacement.
