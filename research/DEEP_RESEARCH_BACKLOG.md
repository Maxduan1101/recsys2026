...
6. **LightGBM/CatBoost LambdaRank design**
   ...
   - Implemented `judge_brief`, `judge_compact_mix`, and `judge_clean_mix` response styles. After duplicate/list-valued title and artist display cleanup, `judge_clean_mix` keeps the same ranking; the four-model ensemble variant reaches official dev lexical `0.20061` and Blind A local Distinct-2 `0.60724`.
   - Four-model ensemble high-lexical backups after title/artist display cleanup: clean compact reaches official dev lexical `0.21139` / Blind A Distinct-2 `0.67799`; compact-broad reaches `0.22276` / `0.69915` but is more mechanical.
   - Pro round6 recommended response LLM-judge optimization before further risky ranking work. Implemented `judge_planned` and `judge_balanced_mix`; the balanced natural backup keeps the four-model ranking with official dev lexical `0.17719` and Blind A Distinct-2 `0.55978`, while `judge_planned` reads more natural but drops lexical to `0.11423` / `0.39854`.
