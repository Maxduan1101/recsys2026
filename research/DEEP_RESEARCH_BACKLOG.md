...
6. **LightGBM/CatBoost LambdaRank design**
   ...
   - Implemented `judge_brief`, `judge_compact_mix`, and `judge_clean_mix` response styles. After duplicate/list-valued title display cleanup, `judge_clean_mix` keeps the same ranking, raises official dev lexical diversity to `0.20061`, and keeps Blind A local Distinct-2 at `0.60453`; the four-model ensemble variant reaches official dev lexical `0.20049` and Blind A local Distinct-2 `0.60816`.
   - Four-model ensemble high-lexical backups after title display cleanup: clean compact reaches official dev lexical `0.21123` / Blind A Distinct-2 `0.67851`; compact-broad reaches `0.22259` / `0.69962` but is more mechanical.
   - Pro round6 recommended response LLM-judge optimization before further risky ranking work. Implemented `judge_planned` and `judge_balanced_mix`; the balanced natural backup keeps the four-model ranking with official dev lexical `0.17710` and Blind A Distinct-2 `0.56036`, while `judge_planned` reads more natural but drops lexical to `0.11423` / `0.39854`.
