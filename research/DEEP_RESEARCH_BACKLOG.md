...
6. **LightGBM/CatBoost LambdaRank design**
   ...
   - Implemented `judge_brief`, `judge_compact_mix`, and `judge_clean_mix` response styles. `judge_clean_mix` keeps the same ranking, raises official dev lexical diversity to `0.20032`, and raises Blind A local Distinct-2 to `0.61010`; the ensemble variant reaches official dev lexical `0.20002` and Blind A local Distinct-2 `0.61335`.
