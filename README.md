...
Public Blind A feedback from one conservative submission was `nDCG@20=0.1935`, `catalog_diversity=0.0257`, `lexical_diversity=0.0125`, and `llm_judge_score=1.0`. Blind A currently has only 80 rows, so the maximum possible catalog diversity is `1600 / 47071 = 0.0340`; catalog is not the main bottleneck. After OOF validation and LTR tuning, the immediate single-model candidate is `goalflow_ltr120_lambda2_head0_judge_clean_mix_clean`, the lower-risk mixed text fallback is `goalflow_ltr120_lambda2_head0_judge_mix_clean`, the fixed-style conservative text fallback is `goalflow_ltr120_lambda2_head0_judge_v2_clean`, the micro-gain ensemble candidate is `goalflow_ens_ltr120_140_200_lambda2_rrf60_judge_clean_mix_clean`, `goalflow_ltr120_lambda2_head0_compact_broad_clean` is the higher-Distinct-2 mechanical text backup, `goalflow_segcat_ltr120_140_200_ens_judge_v2_clean` is a high-risk segment-selection experiment, and `goalflow_head20_compact_broad` remains the conservative legacy-rank backup.
